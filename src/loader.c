...
#if LAB >= 2
	//terminal_printf("vemos....\n");
	//terminal_init();

	// 1. Save the pointer to an array descriptor memory areas
	// 2. Store the number of elements in the array;
	struct bios_mmap_entry *mm = (struct bios_mmap_entry *)BOOT_MMAP_ADDR;
	uint32_t cnt = *((uint32_t *)BOOT_MMAP_ADDR - 1);

	// 3. Initializing the terminal
	terminal_init();

	uint64_t kernel_entry_point;

	// 4. Read the kernel headers of elf file ENTER HERE
	if (loader_read_kernel(&kernel_entry_point) != 0)
		//terminal_printf("si entro aqui es pq hubo error en load_read_kernel");
		//goto something_bad;

	//5. Determine the amount of available memory
	loader_detect_memory(mm, cnt);
	//terminal_printf("hello !!!\n");
	int mmx,mmx2;
	//6. Initialization of available memory;
	mmx=loader_init_memory(mm, cnt);
	terminal_printf("1_loader value is: %d \n", mmx);
	if (mmx == -1)
		{
		 mmx2=loader_init_memory(mm, cnt);
		 terminal_printf("2_loader value is: %d \n", mmx2);
...
// TASK n°2 ~~~ TASK n°2 ~~~ TASK n°2 ~~~ TASK n°2 ~~~ TASK n°2 ~~~
void *loader_alloc(uint64_t size, uint32_t align)
{
	// Выделение памяти. Задание No2

	// LAB 2: Your code here:
    //      Step 1: round loader_freemem up to be aligned


	uint8_t *loader_freemem; // 8-bit unsigned type ---byte 255

	// uint64_t loader_freemem = ROUND_UP(len, PAGE_SIZE);
	//      Step 2: save current value of loader_freemem as allocated chunk

	loader_freemem = (void *)ROUND_UP((uint32_t)free_memory, align);
	...
	/*
	 ...
	 struct elf64_header *elf_header = loader_alloc(sizeof(*elf_header), PAGE_SIZE);
		...
		if (disk_io_read_segment((uint32_t)elf_header, ATA_SECTOR_SIZE, 1) != 0)
			terminal_printf("no cargo el kernel 1\n");

		if (elf_header->e_magic != ELF_MAGIC)
			terminal_printf("no cargo el kernel 2\n");
		...
		for (struct elf32_program_header *ph = ELF32_PHEADER_FIRST(elf_header);
			...
			if (disk_io_read_segment(ph->p_va, ph->p_memsz, lba) != 0)
				terminal_printf("no cargo el kernel 3\n");
		}
...
int loader_init_memory(struct bios_mmap_entry *mm, uint32_t cnt)
{
	...
	if (loader_map_section(KERNEL_BASE, 0x0, ROUND_DOWN(max_physical_address, PAGE_SIZE), false) != 0)
		{terminal_printf("erro5\n");
		//return -1;
		}
	//terminal_printf("erro6\n");
	return 0;
}
