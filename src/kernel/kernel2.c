...
void kernel_init_mmap(void)
{
    ...
    struct cpu_context *cpu = cpu_context(); //definido en cpu.h como una estructura con dato tipo  page map level 4 entry
    									     // y una estructura de de tipo puntero de tareas y un puntero a la tera
    static struct mmap_state state;    // una estr def en map.h de punt a paginas, el cont de paginas y un punt a pag libres
    ...
    cpu->pml4 = config->pml4.ptr; //page map label four table pointer

    // Reinitialize state
    // see state struct info
    // use config param
    state.free = (struct mmap_free_pages){ NULL }; // el puntero a paginas libres apuntando a paginas libres
    // LAB3 code here

    state.pages_cnt = config->pages_cnt;  // pero las paginas pasads como parametro las toma
    state.pages = config->pages.ptr;      // y la cantidad
    mmap_init(&state);
