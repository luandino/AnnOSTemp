...
static const char *interrupt_name[256] = {
	...
	[INTERRUPT_VECTOR_SYSCALL] = "syscall", //NEW !!!
...
void interrupt_handler(struct task_context ctx)
{
	...
	case INTERRUPT_VECTOR_BREAKPOINT: {// 3
	...
	case INTERRUPT_VECTOR_PAGE_FAULT: // 14
		return page_fault_handler(cpu->task);
	case INTERRUPT_VECTOR_DIV_BY_ZERO: // 0
	case INTERRUPT_VECTOR_DEBUG: //1
	case INTERRUPT_VECTOR_NMI://2
	case INTERRUPT_VECTOR_OVERFLOW://4
	case INTERRUPT_VECTOR_BOUND_RANGE://5
	case INTERRUPT_VECTOR_IVALID_OPCODE: //6
	case INTERRUPT_VECTOR_DEVICE_NOT_AVAILABLE: //7
	case INTERRUPT_VECTOR_DOUBLE_FAULT: //8
	case INTERRUPT_VECTOR_INVALID_TSS: //10
	case INTERRUPT_VECTOR_SEGMENT_NOT_PRESENT: //11
	case INTERRUPT_VECTOR_STACK: //12
	case INTERRUPT_VECTOR_GENERAL_PROTECTION: //13
	case INTERRUPT_VECTOR_X86_FP_INSTRUCTION: //16
	case INTERRUPT_VECTOR_ALIGNMENT_CHECK: //17
	case INTERRUPT_VECTOR_MACHINE_CHECK: //18
	case INTERRUPT_VECTOR_SIMD_FP: //19
	case INTERRUPT_VECTOR_SECURITY_EXCEPTION: //30
	case INTERRUPT_VECTOR_SYSCALL:
		return syscall(cpu->task); // in syscall.c
...
void interrupt_init(void)
{
	...
	// kernel/misc/gdt.h #define INTERRUPT_GATE(sel_, off_, ist_, dpl_) GATE(sel_, ((uint64_t)off_), ist_, TYPE_INTERRUPT_GATE, dpl_)
	...
	// kernel/lib/memory/layout.h
	// KERNEL_STACK_TOP	(KERNEL_BASE)
	// KERNEL_STACK_SIZE	(PAGE_SIZE * 8)
	// KERNEL_INFO		(KERNEL_STACK_TOP - KERNEL_STACK_SIZE - PAGE_SIZE)
	// KERNEL_BASE in Makefile
